...
- Улучшение DHUD: теперь можно использовать каналы в DHUD сообщениях (vanilla bug)
...
- Исправлено некорректное выравнивание текста по правому краю в VGUI-меню (например, в меню закупки) (vanilla bug) (@YoshiokaHaruki)
...
- Исправлена ошибка, из-за которой демо-записи могли начинаться с неправильного момента времени (vanilla bug)
...
- Исправлено переполнение буфера при чтении пустых строк в файле `sentences.txt` (vanilla bug) (@scriptedsnark)
- Исправлено 'залипание' снайперского прицела при макс. увеличении (zoom), особенно заметное на мышах с высоким DPI и высоком FPS (vanilla bug) (@hajimura) [ссылка](https://youtu.be/8Z09xvpNQ3E?t=97)
...
- Исправлена клиентская анимация Deploy вызванная предсказыванием оружий (vanilla bug)
- Исправлен сбой из-за `AllocBlock: full` (vanilla bug)
- Исправлен сбой из-за недействительных user-messages от `StatusIcon` and `DeathMsg` (vanilla bug)
- Исправлен `MOTD`, когда в пути используется кириллица (vanilla bug)
- Исправлен сбой карт, где `MAX_MAP_LEAFS` был больше заданного лимита `1024`, что приводило к неопределенному поведению в клиенте (vanilla bug)
- Исправлена некорректная позиция игроков/сущностей в режиме просмотры карты (vanilla bug)
- Исправлена утечка памяти `SPR_GetList` (vanilla bug)
- Исправлена утечка памяти `DetailTextures` с `r_detailtextures 0` (vanilla bug)
- Исправлена утечка памяти в системе коротких фраз VOX (vanilla bug)
...
- Исправлен сбой клиента, который мог произойти при изменении настроек видео (vanilla bug)
...
- Исправлен сбой, связанный с переключением режимов отображения в настройках видео (vanilla bug)
